...
- [x] Server: Add filters support for all GET/POST requests
...
- [x] API: Basic auth routes `/sign-in`, `/sign-out`, `/register`, `/refresh`
...
- [ ] FE: Loading and page transitions improvements. Move data fetching into the component (avoid resolvers). Display loading states for data fragments loaded from the server.
- [ ] FE: Implement words list smart pagination on scroll. Wrap all words with auto-size virtual scroll, implement segments fetching one by one.
...
- [x] Create simple python library for DB communication
- [x] Add `.env` file read from the Python script to connect to DB
- [x] Split prompts into different py/txt files
- [x] Add simple CLI wrapper around python `/scripts` and move them into `/tools`
- [ ] Add precocious before data insertion (check DB for existing records of Words, Meanings, Examples, etc.)
- [x] Include python scripts into git (remove `.gitignore` scripts folder)
